                 ...
                 const struct drm_ccmd *ccmd_dispatch, unsigned int dispatch_size)
{
   ...
   /* get_device_fd: not in this vendored virgl_context vtable; unused by core. */
   (void)drm_context_get_device_fd;
...
drm_context_blob_id_valid(struct drm_context *dctx, uint32_t blob_id)
{
   ...
   if (_mesa_hash_table_search(dctx->blob_table, (void *)(uintptr_t)blob_id))
      return false;
...
drm_context_retrieve_object_from_blob_id(struct drm_context *dctx, uint64_t blob_id)
{
   ...
   uint32_t id = blob_id;
   struct hash_entry *entry = _mesa_hash_table_search(dctx->blob_table, (void *)(uintptr_t)id);
   if (!entry)
      return NULL;
...
{
   const struct hash_entry *entry = _mesa_hash_table_search(dctx->resource_table, (void *)(uintptr_t)res_id);
...
{
   return !_mesa_hash_table_search(dctx->resource_table, (void *)(uintptr_t)res_id);
}
