...
{
   /* Vendored core: _mesa_hash_table_u64_destroy takes no per-entry callback
    * (upstream passes virgl_fence_table_cleanup_cb). Close fds by walking the
    * inner table first, then destroy. */
   if (virgl_fence_table) {
      hash_table_foreach(virgl_fence_table->table, he) {
         virgl_fence_table_cleanup_cb(he);
      }
   }
...
virgl_fence_set_fd_locked(uint64_t fence_id, int fd)
{
   ...
   /* Walk all fences and retire signaled ones. Vendored core lacks
    * hash_table_u64_call_foreach; iterate the inner table and defer removes
    * (retire_cb removes from the table, which is unsafe mid-iteration). */
   {
      uint64_t to_retire[64];
      unsigned n = 0;
      hash_table_foreach(virgl_fence_table->table, he) {
         struct virgl_fence *f = he->data;
         if (f && n < ARRAY_SIZE(to_retire))
            to_retire[n++] = f->id;
      }
      for (unsigned i = 0; i < n; i++) {
         struct virgl_fence *f =
            _mesa_hash_table_u64_search(virgl_fence_table, to_retire[i]);
         if (f)
            virgl_fence_table_retire_cb(NULL, f, NULL);
      }
   }
